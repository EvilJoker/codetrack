...
function activate(context) {
    ...
    // 初始化设置
    globalCache_1.globalCache.path = context.extensionPath;
    // 加载数据 
    (0, filterView_1.loadProblems)(globalCache_1.globalCache.path);
    // 注册 WebviewView
...
class FilterViewProvider {
    ...
    refresh() {
        // 加载数据
        loadProblems(globalCache_1.globalCache.path);
        // 发送消息给其他视图，让它们自行刷新
    ...
    getHtmlForWebview() {
        return `
            ...
            <body>
                <div class="container">
                    <label>问题路径：${globalCache_1.globalCache.path}/problems</label>
                    <hr>
                    <label>推荐：</label>
...
function loadProblems(extensionPath) {
    const problems = [];
    const problemListPath = path_1.default.join(extensionPath, 'problems');
